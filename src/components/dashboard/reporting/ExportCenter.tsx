...
1. Win Rate: ${exportData.summary.winRate.toFixed(1)}% - ${
  exportData.summary.winRate >= 60 ? 'Excellent performance' :
  exportData.summary.winRate >= 50 ? 'Good performance' :
  'Room for improvement'
}

2. Risk Management: Average RR of ${exportData.summary.avgRR.toFixed(2)} indicates ${
  exportData.summary.avgRR >= 1.5 ? 'strong' :
  exportData.summary.avgRR >= 1 ? 'adequate' : 'poor'
} risk-reward discipline.

3. Consistency: Score of ${exportData.performance.consistencyScore.toFixed(1)}% suggests ${
  exportData.performance.consistencyScore >= 80 ? 'highly consistent' :
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="bg-gradient-to-br from-gray-900 to-gray-800 border-gray-700 hover:border-green-600/50 transition-all duration-300 group">
          <CardContent className="p-6 text-center">
            <FileSpreadsheet className="h-12 w-12 text-green-400 mx-auto mb-4 group-hover:animate-bounce" />
            <h3 className="text-lg font-semibold text-white mb-2">Excel Report</h3>
            <p className="text-sm text-gray-400 mb-4">
              Spreadsheet with all trade data and performance metrics
            </p>
            <Button
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-4 bg-gray-800/50 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-400">
              <div>• Performance summary metrics</div>
              <div>• Complete trade history</div>
              <div>• Risk analysis data</div>
              <div>• Consistency scores</div>
              {includeAI && (
                <>
      ...
      {lastExport && (
        <Card className={`bg-gradient-to-br from-gray-900 to-gray-800 border-gray-700 ${
          lastExport.includes('failed') ? 'border-red-700' : 'border-green-700'
        }`}>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <p className={`text-sm ${
                lastExport.includes('failed') ? 'text-red-400' : 'text-green-400'
              }`}>
                {lastExport}
              </p>
            </div>
      ...
      <Card className="bg-gradient-to-br from-gray-900 to-gray-800 border-gray-700">
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-400">
            ...
            <div>
              <h5 className="text-white font-medium mb-2">Excel/CSV Files</h5>
              <p>Perfect for further analysis in spreadsheet software or importing into other tools.</p>
            </div>
          </div>
        </CardContent>
      </Card>
