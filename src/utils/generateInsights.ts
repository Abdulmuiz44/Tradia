 ...
 */
export function generateInsights(trades: TradeLike[], options?: {
  ...
  const parsedPLs = trades.map(t => parsePL(t.pnl ?? t.profit ?? t.prof || 0));
  const netPL = parsedPLs.reduce((s, v) => s + v, 0);
  const avgPL = total ? netPL / total : 0;
  const wins = trades.filter(t => String((t.outcome ?? "").toLowerCase()).includes("win") || parsePL(t.pnl) > 0).length;
  ...
  insights.push({
    id: "summary",
    text: `Overall: ${total} trades · Win rate ${globalWinRate.toFixed(1)}% · Avg P/L ${avgPL >= 0 ? "+" : ""}${avgPL.toFixed(2)}.`,
    severity: "info",
    details: { total, globalWinRate, avgPL, netPL }
  });

  // 1) Symbol-level performance & hotspots by time-of-day
  const symbolMap = new Map<string, { trades: TradeLike[]; wins: number; total: number; netPL: number }>();
  trades.forEach(t => {
    const s = (t.symbol ?? "UNKNOWN").toUpperCase();
    const pl = parsePL(t.pnl ?? t.profit ?? 0);
  ...
  const symbolStats = Array.from(symbolMap.entries()).map(([sym, { trades:ts, wins: w, total:tot, netPL:npl }]) => ({
    symbol: sym,
    total: tot,
    wins: w,
  ...
  const notableSymbols = symbolStats.filter(s => s.total >= Math.max(minTradesForPattern, 3) && s.winRate >= Math.max(60, globalWinRate + winRateDeltaPct));
  if (notableSymbols.length) {
    notableSymbols.slice(0,3).forEach((s) => {
      insights.push({
        id: `symbol-${s.symbol}`,
        text: `High performer: ${s.symbol} → ${s.winRate.toFixed(1)}% win rate (${s.total} trades), avg P/L ${s.avgPL.toFixed(2)}. Consider focusing sizing/edge here.`,
        severity: "recommendation",
        details: s
      });
    });
  } else {
    const worst = symbolStats.slice(-3);
    // show underperformers also
    const under = symbolStats.filter(s => s.total >= minTradesForPattern && s.winRate <= Math.min(40, globalWinRate - winRateDeltaPct));
    under.slice(0,3).forEach(s => {
      insights.push({
        id: `symbol-low-${s.symbol}`,
        text: `Weak performer: ${s.symbol} → ${s.winRate.toFixed(1)}% win rate (${s.total} trades). Review entries, timeframe, or avoid scaling this symbol.`,
        severity: "warning",
        details: s
      });
    });
  ...
  const bucketMap = new Map<BucketKey, { symbol: string; bucketIndex: number; trades: TradeLike[]; wins: number; total: number; netPL: number }>();
  trades.forEach(t => {
    const s = (t.symbol ?? "UNKNOWN").toUpperCase();
    const hour = hourOf(t.openTime ?? t.openTime ?? t.open_time ?? t.time ?? "");
    if (hour === null) return;
  ...
  const bucketStats = Array.from(bucketMap.values()).map(b => ({
    symbol: b.symbol,
  ...
  const interestingBuckets = bucketStats.filter(b => b.winRate >= Math.max(60, globalWinRate + winRateDeltaPct)).sort((a,b) => b.winRate - a.winRate);
  if (interestingBuckets.length) {
    ...
    insights.push({
      ...
      text: `Timing edge: For ${top.symbol}, your win rate is ${top.winRate.toFixed(1)}% between ${startHour}:00–${endHour}:59 (${top.total} trades). Consider prioritizing this window.`,
      severity: "recommendation",
      details: top
    });
  }
  ...
  const stratStats = Array.from(stratMap.entries()).map(([name, val]) => ({
    strategy: name,
    total: val.total,
  ...
  const goodStrats = stratStats.filter(s => s.total >= Math.max(minTradesForPattern, 3) && s.winRate >= Math.max(55, globalWinRate + (winRateDeltaPct/2)));
  if (goodStrats.length) {
    insights.push({
      id: `strategy-good-${goodStrats[0].strategy}`,
      text: `Strategy highlight: "${goodStrats[0].strategy}" has ${goodStrats[0].winRate.toFixed(1)}% wins over ${goodStrats[0].total} trades. Consider documenting and scaling rules for this strategy.`,
      severity: "recommendation",
  ...
  const emoStats = Array.from(emoMap.entries()).map(([emo, val]) => ({
    emotion: emo,
    count: val.count,
  ...
  if (emoStats.length) {
    ...
    if (best.count >= 3 && best.winRate >= Math.max(60, globalWinRate + 8)) {
      insights.push({
        ...
        text: `Behavioral finding: When you reported being "${best.emotion}", your win rate was ${best.winRate.toFixed(1)}% (n=${best.count}). Aim to replicate the processes that lead to that state.`,
        severity: "recommendation",
        details: best
      });
    }
    if (worst.count >= 3 && worst.winRate <= Math.min(40, globalWinRate - 8)) {
      insights.push({
        ...
        text: `Warning: When you reported being "${worst.emotion}", your win rate dropped to ${worst.winRate.toFixed(1)}% (n=${worst.count}). Consider avoiding trading while in this state or create rules to reduce size.`,
        severity: "warning",
        details: worst
      });
    }
  ...
  insights.push({
    id: "consistency",
    text: `Consistency: your P/L standard deviation is ${plStdDev.toFixed(2)}. Consider position-sizing or stop rules to tighten variability.`,
    severity: "info",
