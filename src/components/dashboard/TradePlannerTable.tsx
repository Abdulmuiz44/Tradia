...
/**
 * Improvements:
 * - LocalPlan extends TradePlan with UI-only fields (notes, tier, createdAt)
 * - All state uses Partial<LocalPlan> so notes/tier are safe
 * - DeleteConfirmModal replaces confirm()
 * - AlertsModal wired to Auto alerts button
 * - GenericModal used by both modals
 * - Loading skeleton when ctx.loading === true
 * - Sleeker transparent inputs (no white boxes), mobile-first layout
 * - Upsell tiers (Plus $9/mo, Pro $19, Elite $39) — matches PricingPlans.tsx
 */
...
export default function TradePlannerTable() {
  ...
  // expected context methods (fallbacks are handled)
  const { plans = [], deletePlan, updatePlan, markExecuted, addPlan, loading = false } = ctx ?? {};
  ...
  // Create plan (maps LocalPlan to TradePlan where possible)
  const handleCreatePlan = () => {
    ...
    const payload: LocalPlan = {
      ...
      // outcome removed (was causing TS error) — keep status instead
    ...
    // call context addPlan/createPlan/setPlans in order of availability
    if (typeof addPlan === "function") {
      ...
      console.warn("No addPlan / createPlan / setPlans method found in TradePlanContext - plan not persisted.");
    }
  ...
  const performDelete = (id: string) => {
    ...
    else if (typeof ctx?.setPlans === "function") ctx.setPlans((prev: LocalPlan[]) => (prev ?? []).filter((p) => p.id !== id));
  ...
  // fallback: when plan structures vary, create a LocalPlan view
  ...
  const cardBase = "bg-white/4 backdrop-blur-sm rounded-md p-3 shadow-sm transition-shadow duration-150 border border-zinc-700";

  /* --- Modals --- */
  const GenericModal: React.FC<{ open: boolean; title?: string; onClose: () => void; children?: React.ReactNode }> = ({ open, title, onClose, children }) => {
    if (!open) return null;
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="fixed inset-0 bg-black/40" onClick={onClose} />
        <div className="relative z-10 w-full max-w-md bg-slate-900 rounded p-4 border border-zinc-700 shadow-lg">
  ...
  const DeleteConfirmModal: React.FC<{ id: string | null; onCancel: () => void; onConfirm: (id: string) => void }> = ({ id, onCancel, onConfirm }) => {
    if (!id) return null;
    return (
      <GenericModal open={!!id} title="Delete plan?" onClose={onCancel}>
        <div className="text-sm text-zinc-300">Are you sure you want to permanently delete this trade plan? This action cannot be undone.</div>
        <div className="mt-4 flex gap-2 justify-end">
          <Button size="sm" variant="ghost" onClick={onCancel}>Cancel</Button>
          <Button size="sm" onClick={() => onConfirm(id)}>Delete</Button>
        </div>
      </GenericModal>
    );
  };
    ...
    return (
      <GenericModal open={!!id} title={`Alerts — ${plan.symbol}`} onClose={onClose}>
        ...
        <div className="mt-3 grid grid-cols-1 gap-2">
          <div className="text-xs text-zinc-400">Alert when price reaches</div>
          <div className="flex gap-2">
            <Input className="bg-transparent border border-zinc-700" placeholder="Price" />
            <Button size="sm" variant="ghost" onClick={() => alert("Saved (demo)")}>Save</Button>
          </div>
        </div>

        <div className="mt-3 flex items-center gap-2">
          <input id="en" type="checkbox" checked={enabled} onChange={() => setEnabled((s) => !s)} />
          <label htmlFor="en" className="text-sm text-zinc-300">Enable email reminders</label>
        </div>
      </GenericModal>
    );
  };
  ...
  /* --- UI: inputs styled to be transparent & bordered (no white boxes) --- */
  const inputClass = "bg-transparent border border-zinc-700 rounded px-2 py-2 text-sm text-white";
    ...
    <div className="w-full mt-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">Plan trades, estimate risk & reward, and keep ideas organized. Upgrade to unlock automation and analytics.</p>
        </div>
        ...
        <div className={cardBase}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="space-y-2">
              ...
              <Input placeholder="BTCUSD" value={newPlan.symbol ?? ""} onChange={(e) => setNewPlan((p) => ({ ...p, symbol: e.target.value }))} className={inputClass} />
            </div>

            <div className="space-y-2">
              <label className="text-xs text-zinc-400">Strategy / Setup</label>
              <Input placeholder="Pullback" value={newPlan.setupType ?? ""} onChange={(e) => setNewPlan((p) => ({ ...p, setupType: e.target.value }))} className={inputClass} />
            </div>

            <div className="space-y-2">
              <label className="text-xs text-zinc-400">Lot Size</label>
              <Input type="number" min={0.01} step={0.01} value={String(newPlan.lotSize ?? 1)} onChange={(e) => setNewPlan((p) => ({ ...p, lotSize: Number(e.target.value) }))} className={inputClass} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-6 gap-3 mt-3">
            <div className="space-y-2">
              ...
              <Input type="number" value={String(newPlan.plannedEntry ?? "")} onChange={(e) => setNewPlan((p) => ({ ...p, plannedEntry: Number(e.target.value) }))} className={inputClass} placeholder="Entry" />
            </div>

            <div className="space-y-2">
              <label className="text-xs text-zinc-400">Stop Loss</label>
              <Input type="number" value={String(newPlan.stopLoss ?? "")} onChange={(e) => setNewPlan((p) => ({ ...p, stopLoss: Number(e.target.value) }))} className={inputClass} placeholder="SL" />
            </div>

            <div className="space-y-2">
              <label className="text-xs text-zinc-400">Take Profit</label>
              <Input type="number" value={String(newPlan.takeProfit ?? "")} onChange={(e) => setNewPlan((p) => ({ ...p, takeProfit: Number(e.target.value) }))} className={inputClass} placeholder="TP" />
            </div>

            <div className="space-y-2">
              <label className="text-xs text-zinc-400">Planned RR</label>
              <Input type="number" value={String(newPlan.riskReward ?? "")} onChange={(e) => setNewPlan((p) => ({ ...p, riskReward: Number(e.target.value) }))} className={inputClass} placeholder="e.g. 2" />
            </div>

            <div className="space-y-2 col-span-1 sm:col-span-2 md:col-span-2">
              <label className="text-xs text-zinc-400">Notes</label>
              <Input value={newPlan.notes ?? ""} onChange={(e) => setNewPlan((p) => ({ ...p, notes: e.target.value }))} className={inputClass} placeholder="Trade idea notes..." />
            </div>
            ...
            <div className="flex items-end gap-2 justify-end">
              ...
              <Button size="sm" variant="ghost" onClick={() => resetNewPlan()}>Reset</Button>
              <Button size="sm" onClick={handleCreatePlan}>Save Plan</Button>
            </div>
          </div>
          ...
          <div className="mt-3 flex gap-2 items-center">
            ...
            {PRESETS.map((p) => (
              <button key={p.label} className="px-3 py-1 text-xs rounded bg-zinc-800 hover:bg-zinc-700" onClick={() => setNewPlan((cur) => ({ ...cur, setupType: p.setupType, riskReward: p.rr ?? cur.riskReward }))}>
                {p.label}
              </button>
            ))}

            <div className="ml-auto text-xs text-zinc-400">Unlock advanced automated alerts in <span className="font-medium capitalize">{userPlan}</span>+</div>
          </div>
        </div>
      )}
          ...
          sortedPlans.map((plan: LocalPlan) => {
            ...
            return (
              <div key={plan.id} className={cn(cardBase, "relative overflow-hidden")}>
                {/* blurred overlay for locked advanced features */}
                {lockedAdvanced && (
                  <div className="absolute inset-0 z-20 flex items-center justify-center" style={{ background: "rgba(2,6,23,0.55)", backdropFilter: "blur(4px)" }}>
                    <div className="text-center text-white space-y-2 px-4">
                      ...
                      <div className="mt-2 flex gap-2 justify-center">
                        <Button size="sm" variant="default" onClick={() => (ctx?.openUpgrade ? ctx.openUpgrade() : (window.location.href = "/pricing"))}>Upgrade</Button>
                        <Button size="sm" variant="ghost" onClick={() => alert("Preview disabled — upgrade to view")}>Learn more</Button>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                      <div>
                        ...
                        {isEditing ? (
                          <Input className={inputClass} type="number" value={String(editableData.plannedEntry ?? "")} onChange={(e) => setEditableData((d) => ({ ...d, plannedEntry: Number(e.target.value) }))} />
                        ) : (
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <Input className={inputClass} type="number" value={String(editableData.stopLoss ?? "")} onChange={(e) => setEditableData((d) => ({ ...d, stopLoss: Number(e.target.value) }))} />
                        ) : (
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <Input className={inputClass} type="number" value={String(editableData.takeProfit ?? "")} onChange={(e) => setEditableData((d) => ({ ...d, takeProfit: Number(e.target.value) }))} />
                        ) : (
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <Input className={inputClass} type="number" value={String(editableData.lotSize ?? plan.lotSize ?? 1)} onChange={(e) => setEditableData((d) => ({ ...d, lotSize: Number(e.target.value) }))} />
                        ) : (
                    ...
                    <div className="mt-3">
                      ...
                      {isEditing ? (
                        <Input className={inputClass} value={String(editableData.notes ?? "")} onChange={(e) => setEditableData((d) => ({ ...d, notes: e.target.value }))} />
                      ) : (
                    ...
                    <div className="flex items-center gap-2">
                      {isEditing ? (
                        ...
                        <>
                          ...
                          <Button size="icon" variant="ghost" onClick={() => confirmDelete(plan.id)}>
              ...
              {locked && (
                <div className="absolute inset-0 z-10 flex items-center justify-center" style={{ background: "rgba(2,6,23,0.55)", backdropFilter: "blur(4px)" }}>
                  <div className="text-center text-white px-4">
                    ...
                    <div className="mt-3">
                      <Button size="sm" onClick={() => (ctx?.openUpgrade ? ctx.openUpgrade() : (window.location.href = "/pricing"))}>Upgrade</Button>
                    </div>
                  </div>
                </div>
              )}
